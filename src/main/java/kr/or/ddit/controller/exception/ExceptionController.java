...
public class ExceptionController {
	 ...
	 *  
	 *  	1-3) 예외처리 불가 기준
	 *  	- 사용자가 정의한 예외 처리기를 거치지 않은 예외로 프레임 워크에서 이러한 예외를 처리한 기본적인 내용은 아래와 같아.
	 ...
	 * 				> Http Status 500
	 *  			> [예외상황 만들 위치]
	 *  				sqlmap/boardMapper_SQL.xml id가 delete인 쿼리에서 board_no를 boar_no2로  수정
	 *  			> [요청 URL]
	 ...
	 *  
	 *  	예외 처리 방법
	 *  	- @ControllerAdvice 어노테이션은 스프링 컨트롤러에서  발생하는 예외를 처리하는 핸들러 클래스 임을 명시한다.
	 ...
	 *  		- D. 존재하지 않는 게시물을 조회할 때, 사용자가 정의한 예외 발생
	 *  
	 *  	- 처리할 수 없는 예외  <404와 500은 되지 않음>
	 *  		- B. 수정 화면 생성할 때 뷰 파일에서 예외 발생
	 *  		- E. 존재하지 않는 페이지 URL 요청 시 예외 발생
	 *  
	 *  7. 예외 정보 출력
	 *  - 예외에 대한 내용을  Model 객체에 이용해서 전달하여 뷰(View) 화면에서 출력이 가능하다.
	 *  
	 *  	예외 처리 방법
	 ...
	 *  		> 입력값 검증 대상인 자바빈즈 메소드 매개변수에 @Validated를 지정하고 바로 다음에 BindingResult를 정의한다. / form:error에 바인딩 됨
