...
public class ImageUpload {
	...
	public String imageUpload(
		...
		//CKEditor 본문 내용에 이미지를 업로드해보면 'upload' 라는 키로 파일 데이터가 전달되는 걸 확인할 수 있다.
			...
			if(file.getContentType().toLowerCase().startsWith("image/")) {
				...
				if(file.getSize() > limitSize) {	//최대 크기 2MB 보다 클 때
					...
					try {
						...
						// 업로드 경로로 설정된 폴더 구조가 존재하지 않는 경우, 파일을 복사할 수 없으므로 폴더 구조가 존재하지 않는 경우 생성하고
						// 존재하는 경우 건너뛰도록 한다.
						 ...
						 * 	{
						 *		"uploaded" : 1,
						 * 		"fileName" : "xxxxx-xxxxxxxxxx.jpg"
						 * 			"url"  : "/resources/img/xxxxxx-xxxxxxxxxx.jpg"
						 * 	}
						 */
